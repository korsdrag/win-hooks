...
auto WINAPI CallWndProc(int, WPARAM, LPARAM) -> LRESULT;
auto WINAPI CBTProc(int, WPARAM, LPARAM) -> LRESULT;
auto WINAPI DebugProc(int, WPARAM, LPARAM) -> LRESULT;
auto WINAPI GetMsgProc(int, WPARAM, LPARAM) -> LRESULT;
auto WINAPI KeyboardProc(int, WPARAM, LPARAM) -> LRESULT;
auto WINAPI MouseProc(int, WPARAM, LPARAM) -> LRESULT;
auto WINAPI MessageProc(int, WPARAM, LPARAM) -> LRESULT;
                ...
                case IDM_MSGFILTER:
                    ...
                    if (!afHooks[index])
                    {
                        myhookdata[index].hhook = SetWindowsHookEx(
                            myhookdata[index].nType, myhookdata[index].hkprc, (HINSTANCE)NULL, GetCurrentThreadId());
    ...
    switch (nCode)
    {
        case HC_ACTION:
            hResult = StringCchPrintf(
                szCWPBuf, 256 / sizeof(TCHAR), "CALLWNDPROC - tsk: %ld, msg: %s, %d times   ", wParam, szMsg, c++);
            if (FAILED(hResult))
            {
...
LRESULT CALLBACK GetMsgProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (nCode)
    {
        case HC_ACTION:
            ...
            hdc     = GetDC(gh_hwndMain);
            hResult = StringCchPrintf(
                szMSGBuf, 256 / sizeof(TCHAR), "GETMESSAGE - wParam: %s, msg: %s, %d times   ", szRem, szMsg, c++);
            if (FAILED(hResult))
            {
                // TODO: write error handler
            }
...
LRESULT CALLBACK DebugProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (nCode)
    {
        case HC_ACTION:
            hResult = StringCchPrintf(
                szBuf, 128 / sizeof(TCHAR), "DEBUG - nCode: %d, tsk: %ld, %d times   ", nCode, wParam, c++);
            if (FAILED(hResult))
            {
                // TODO: write error handler
            }
...
LRESULT CALLBACK CBTProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    hResult =
        StringCchPrintf(szBuf, 128 / sizeof(TCHAR), "CBT -  nCode: %s, tsk: %ld, %d times   ", szCode, wParam, c++);
    if (FAILED(hResult))
    {
        // TODO: write error handler
    }
...
LRESULT CALLBACK KeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    hdc = GetDC(gh_hwndMain);
    hResult =
        StringCchPrintf(szBuf, 128 / sizeof(TCHAR), "KEYBOARD - nCode: %d, vk: %d, %d times ", nCode, wParam, c++);
    if (FAILED(hResult))
    {
        // TODO: write error handler
    }
...
LRESULT CALLBACK MessageProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    hdc = GetDC(gh_hwndMain);
    hResult =
        StringCchPrintf(szBuf, 128 / sizeof(TCHAR), "MSGFILTER  nCode: %s, msg: %s, %d times    ", szCode, szMsg, c++);
    if (FAILED(hResult))
    {
        // TODO: write error handler
    }
